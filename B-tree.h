...
public:
	...
	void print(BNode* n) {
		if (n != NULL) {
			cout <<MAGENTA <<"\n<----------------------------------->" <<RESET <<endl;
			cout <<RED <<"BTree Node Keys: "<<RESET;
			for (int i = 0; i < n->keycount; i++) {
				cout <<YELLOW<<"\n\t--> "<<CYAN<< n->keys[i]->Hkey <<YELLOW <<"\n\t\t->("<<GREEN << n->keys[i]->value << ")"<< RESET;
			}
			cout <<MAGENTA <<"\n<----------------------------------->" <<RESET<< endl;
			cout << endl;
	...
	void searchall(BNode* n, FileNode**& f, int& s, string val) {
		if (n != NULL) {
			for (int i = 0; i < n->keycount; i++) {
					if (f == NULL) {
						f = new FileNode * [1];
	...
	//..................................................................................
	void deletionmachineB(BTree* prevB, string comparitor, string home) {
		FileNode** f = NULL;
		int sizef = 0;

		searchall(root, f, sizef, comparitor);
		...
		for (int i = 0; i < sizef; i++) {
			...
			std::string copypath = base_directory + "\\" + home;
			cout << "\t=====>" << copypath <<"                       " << f[i]->value << endl;
			
			copyFileToDirectory(f[i]->value, copypath);



			std::string removepath = filename_extractions(f[i]->value);
			removepath = base_directory + "\\" + comparitor + "\\" + removepath;
	...
	void additionmachineB(BTree* prevB, string comparitor, string home) {
		FileNode** f = NULL;
		int sizef = 0;
		...
		for (int i = 0; i < sizef; i++) {
			...
			std::string copypath = base_directory + "\\" + comparitor;
			cout << "\t=====>" << copypath << "    " << f[i]->value << endl;
			std::string removepath = filename_extractions(f[i]->value);
			removepath = base_directory + "\\" + home + "\\" + removepath;

			copyFileToDirectory(removepath, copypath);
	...
	//
	void deleteNode(string val, string path = " ") {
		bool pathcheck = true;
		if (path == " ") {
			pathcheck = false;
		}
		...
		while (temp != NULL) {
			...
			for (int i = 0; i < temp->keycount; i++) {
				if (pathcheck) {
					if (temp->keys[i]->Hkey == val && path == temp->keys[i]->value) {
						foundindex = i;
						break;
					}
				}
		...
		if (temp == NULL) {
			...
			if (temp->isLeaf == 1) {
				for (int i = foundindex; i < temp->keycount - 1; i++) {
					...
					temp->keys[i]->value = temp->keys[i + 1]->value;
				}
				temp->keycount--;
			}
			else {
